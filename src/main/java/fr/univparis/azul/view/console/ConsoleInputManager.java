...
public class ConsoleInputManager extends InputManager {
	private Scanner sc;
	public ConsoleInputManager(GameRenderEngine g) {
		super(g);
	...
	public void read(Player player) {
		...
		if (s == "C") {
			...
			Factory f;
			do {
				f = player.gameBoard.factories.get(Integer.parseInt(question("What facotry do you want to use ? insert a number from 0 to " + (pos2.length - 1), pos2)));
				if(f.isEmpty())
